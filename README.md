...
```text
[Flash 3.5] │ [⚡Fast/📋Plan/👥Team/💬Grill] │ 0627 git:(master) 👤 your.email@gmail.com
API: Flash(*M/H/L)/Pro(H/L) █████░░░░░ 48% (⏰ 3h 52m) | GPT-OSS/Opus/Sonnet ██████████ 100%
```
*(實際輸出包含 24-bit TrueColor ANSI 著色，能依據額度健康度顯示 天空藍 / 亮綠色 / 警示黃 / 亮粉紅。若啟用了 Fast Mode，`⚡Fast` 會以亮藍色動態點亮。)*
...
* ⚡ **動態指令與狀態高亮 (Dynamic Commands & Highlighting)**：
  * 常駐展示四大常用代理人指令提示 `[⚡Fast/📋Plan/👥Team/💬Grill]`（/fast、/planning、/teamwork-preview、/grill-me）。
  * **動態偵測 Fast 模式**：自動讀取本地 `settings.json` 組態，當啟用 Fast 模式時，**`⚡Fast` 會動態高亮點亮為天空藍**，其餘保持低調灰色。
* 🖥️ **雙行輸出設計 (Two-line Layout)**：將系統與 Git 資訊、指令提示以及登入帳號至於第一行，第二行專注顯示 API 額度與進度條，更適合多行提示字元或分欄顯示。
